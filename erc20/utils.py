...
def Base32Encode5Bytes(md5):
    alphabet = "0123456789abcdefghjkmnpqrstvwxyz"
    s0 = alphabet[(md5[0] >> 3) & 0x1F]
    s1 = alphabet[((md5[0] << 2) & 0x1C) | ((md5[1] >> 6) & 0x03)]
    s2 = alphabet[(md5[1] >> 1) & 0x1F]
    s3 = alphabet[((md5[1] << 4) & 0x10) | ((md5[2] >> 4) & 0x0F)]
    s4 = alphabet[((md5[2] << 1) & 0x1E) | ((md5[3] >> 7) & 0x01)]
    s5 = alphabet[(md5[3] >> 2) & 0x1F]
    s6 = alphabet[((md5[3] << 3) & 0x18) | ((md5[4] >> 5) & 0x07)]
    s7 = alphabet[(md5[4] & 0x1F)]
    return s0 + s1 + s2 + s3 + s4 + s5 + s6 + s7

def Base32Encode(md32):
    crc = crc24q(md32, 32)
    strBase32 = ''
    for i in range(6):
        strBase32 += Base32Encode5Bytes(md32[i*5:])
    tail = [md32[30], md32[31],(crc >> 16) % 256, (crc >> 8) % 256, crc % 256]
...
def Base32Decode5Bytes(psz):
    ...
    idx = [0, 0, 0, 0, 0, 0, 0, 0]
    md5 = [0, 0, 0, 0, 0]
    idx[0] = digit[ord(psz[0])]
    idx[1] = digit[ord(psz[1])]
    idx[2] = digit[ord(psz[2])]
    idx[3] = digit[ord(psz[3])]
    idx[4] = digit[ord(psz[4])]
    idx[5] = digit[ord(psz[5])]
    idx[6] = digit[ord(psz[6])]
    idx[7] = digit[ord(psz[7])]
...
def Base32Decode(strBase32):
    if len(strBase32) != 56 :
        return False
    data = []
    for i in range(7):
        data += Base32Decode5Bytes(strBase32[i*8:i*8+8])        
    return data[0:32]
...
def Addr2Hex(addr):
    ...
    return hexlify(bytes(t + Base32Decode(addr[1:])[::-1])).decode()
